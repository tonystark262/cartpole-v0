...
class dqn(object):
    ...
    def create_nn(self):
        '''
        s1 = {1: [self.input_size, 128], 2: [128, 256], 3: [256, 512], 4: [512, 256], 5: [256, 128],
              6: [128, self.output_size]}
        s2 = {1: [128], 2: [256], 3: [512], 4: [256], 5: [128], 6: [self.output_size]}
        for i in s1:
        ...
        '''
        self.weights[1] = tf.Variable(tf.truncated_normal([self.input_size, 20]), name='w1')
        self.weights[2] = tf.Variable(tf.truncated_normal([20, self.output_size]), name='w2')
        self.biases[1] = tf.Variable(tf.truncated_normal([20]), name='b1')
        self.biases[2] = tf.Variable(tf.truncated_normal([self.output_size]), name='b2')
        self.target_weights[1] = tf.Variable(tf.truncated_normal([self.input_size, 20]), name='tw1')
        self.target_weights[2] = tf.Variable(tf.truncated_normal([20, self.output_size]), name='tw2')
        self.target_biases[1] = tf.Variable(tf.truncated_normal([20]), name='tb1')
        self.target_biases[2] = tf.Variable(tf.truncated_normal([self.output_size]), name='tb2')
